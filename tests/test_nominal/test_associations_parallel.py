...
def test_return_type_check(iris_df):
    assoc = associations(iris_df, multiprocessing=True,
    ...
    assert isinstance(
        assoc['corr'], pd.DataFrame), 'assoc["corr"] should be a pandas DataFrame'
    assert isinstance(
        assoc['ax'], matplotlib.axes.Axes), 'assoc["ax"] should be a matplotlib Axes'


def test_dimension_check(iris_df):
    corr = associations(iris_df, multiprocessing=True,
                        max_cpu_cores=MAX_CORE_COUNT)['corr']
...
def test_bad_nom_nom_assoc_parameter(iris_df):
    with pytest.raises(ValueError, match='is not a supported'):
        associations(iris_df, nom_nom_assoc='bad_parameter_name',
                     multiprocessing=True, max_cpu_cores=MAX_CORE_COUNT)
...
def test_mark_columns(iris_df):
    corr = associations(iris_df, mark_columns=True,
                        multiprocessing=True, max_cpu_cores=MAX_CORE_COUNT)['corr']
...
def test_udf(iris_df):
    corr1 = associations(
        iris_df, plot=False, num_num_assoc='pearson', nom_num_assoc='correlation_ratio', multiprocessing=True, max_cpu_cores=MAX_CORE_COUNT)['corr']
    corr2 = associations(
        iris_df, plot=False, num_num_assoc=pr, nom_num_assoc=correlation_ratio, multiprocessing=True, max_cpu_cores=MAX_CORE_COUNT)['corr']
...
def test_datetime_data():
    ...
    df = pd.DataFrame({'dates': result, 'up': nums,
                      'down': sorted(nums, reverse=True)})
    ...
    corr = associations(df, plot=False, multiprocessing=True,
                        max_cpu_cores=MAX_CORE_COUNT)['corr']
    assert corr.compare(
