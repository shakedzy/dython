...
parametrize_sampling_funcs = pytest.mark.parametrize("func", [boltzmann_sampling, weighted_sampling])
...
def test_k_number(func, population, k):
    ...
    assert type(result) == type(population), "Sampling with k != None should return same type as input"
